...
class Feeder:
    def __init__(self, device_id, features=None):
        self.id = device_id
...
class_Divide_Dict = {"Controller": list(), "Source": list(), "Load": list(), "Feeder": list()}


def normalize_role(role):
    """Normalize role values that may arrive as ints or strings."""
    if isinstance(role, str):
        ...
        return role_map.get(cleaned, cleaned)
    return role


def feeder_by_id(device_id):
    for feeder in class_Divide_Dict.get("Feeder", []):
        if feeder.id == device_id:
            return feeder
    return None


def ensure_feeder_registered(device_id, features=None, reason="message"):
    existing = feeder_by_id(device_id)
    ...
    newInstance = Feeder(device_id, features or [])
    class_Divide_Dict.setdefault("Feeder", []).append(newInstance)
    print(f"Registered Type III Feeder from {reason}: {device_id}")
    return newInstance


def source_by_id(device_id):
    for src in class_Divide_Dict.get("Source", []):
        if src.id == device_id:
            return src
    return None
...
def controller_exists():
    return len(class_Divide_Dict.get("Controller", [])) > 0


JUNCTION_NODE_DEVICE_IDS = {"junction-node-sbc-1"}


def looks_like_feeder_device_id(device_id):
    if not device_id:
        return False
    if device_id in JUNCTION_NODE_DEVICE_IDS:
        return True
    did = str(device_id)
    return did.startswith("junction-node") or did.startswith("junction_")
...
def generator_state_change_request(targetDeviceID, toLevel, fromLevel = "ESSL_ANY"):
    energyStartStopRequestWriter.instance.set_dictionary({
        "requestId": {
            'requestingDeviceId': "Sebastians Brain",       # This would need to be changed if you are running a main/redundant controller architecture
...
def send_power_switch(target_device_id, port_number, open_state, seq):
    payload = {
        "requestId": {
    ...
    powerSwitchWriter.instance.set_dictionary(payload)
    powerSwitchWriter.write()
    
    print("Sent to:", payload["requestId"]["targetDeviceId"],"port:", payload["requestId"]["portNumber"])
...
with rti.open_connector(
    ...
    powerSwitchWriter = connector.get_output("TmsPublisher::PowerSwitchRequestWriter")


    print("Writing DeviceInfo.")

    # Set single field using the instance methods; for deviceInfo TOPIC
    deviceInfoWriter.instance.set_string("deviceId", "controller")  
    ...
    activeDiagnosticStateWriter.instance.set_string("deviceId", "controller")
    ...
    for i in range(1, sys.maxsize):
        # Disabled hardcoded feeder test commands while debugging message reception.
        # Re-enable only after feeder discovery and telemetry are confirmed.
        # if i == 50:
        #     send_power_switch("junction-node-sbc-1", 0, True, 1)
        #
        # if i == 100:
        #     send_power_switch("junction-node-sbc-1", 0, False, 2)
        
        


        # Heartbeats being written every iteration
        heartbeatWriter.instance.set_string("deviceId", "controller")
        ...
        try:
            ...
            for sample in deviceInfoReader.samples.valid_data_iter:   # Sample = single device's info
                ...
                role = normalize_role(deviceInfo["role"])
                ...
                if role == 1:
                    ...
                    if not controller_exists():
                        ...
                        class_Divide_Dict["Controller"].append(newInstance)
                        print("Registered Controller:", device_id)


                # -------- SOURCE (Generator) --------
                elif role == "ROLE_SOURCE":

                    if source_by_id(device_id) is None:
                        maxRealPowerAdjusted = deviceInfo["powerDevice"]["source"]["loadSharing"]["maxRealPower"] * 0.8

                        newInstance = Source(
                            maxRealPowerAdjusted,
                            "ESSL_UNKNOWN",
                            deviceInfo["powerHardware"]["fuel"]["maxFuelLevel"],
                            deviceInfo["powerHardware"]["fuel"]["lowFuelLevelCutoff"],
                            device_id,
                            deviceInfo["powerHardware"]["fuel"]["maxFuelLevel"]
                        )

                        class_Divide_Dict["Source"].append(newInstance)
                        print("Registered Source:", device_id)
                ...
                # -------- LOAD --------
                    ...
                    newInstance = Load(
                        deviceInfo["powerDevice"]["load"]["maxRealPower"],
                        deviceInfo["powerDevice"]["load"]["maxReactivePower"],
                        0,
                        0
                    )

                    class_Divide_Dict["Load"].append(newInstance)
                    print("Registered Load:", device_id)
                ...
                # -------- TYPE III FEEDER --------
                    ...
                    features = []
                    try:
                        features = deviceInfo["powerDevice"]["distribution"]["features"]
                    except KeyError:
                        pass

                    ensure_feeder_registered(device_id, features, reason="DeviceInfo")


        except Exception as e:
            print(f"DeviceInfoReader error: {e}")
        ...
        try:
            ...
            for sample in heartbeatReader.samples.valid_data_iter:
                ...
                hb_device_id = heartbeat.get("deviceId")
                if hb_device_id and hb_device_id != "controller":
                    if looks_like_feeder_device_id(hb_device_id):
                        feeder = feeder_by_id(hb_device_id)
                        if feeder is None:
                            feeder = ensure_feeder_registered(hb_device_id, reason="Heartbeat")
                        feeder.last_seen = time.time()
        except Exception as e:
            print(f"HeartbeatReader error: {e}")
        ...
        try:
            ...
            for sample in activeDiagnosticStateReader.samples.valid_data_iter:
                ...
                diag_device_id = activeDiagnosticState.get("deviceId")
                if diag_device_id and diag_device_id != "controller":
                    if looks_like_feeder_device_id(diag_device_id):
                        ensure_feeder_registered(diag_device_id, reason="ActiveDiagnosticState").last_seen = time.time()
        ...
        try:
            ...
            for sample in groundingCircuitStateReader.samples.valid_data_iter:
                ...
                grounding_device_id = groundingCircuitState.get("deviceId")
                if grounding_device_id and grounding_device_id != "controller":
                    if looks_like_feeder_device_id(grounding_device_id):
                        ensure_feeder_registered(grounding_device_id, reason="GroundingCircuitState").last_seen = time.time()
        ...
        try:
            ...
            for sample in powerPortStateReader.samples.valid_data_iter:
                ...
                port_device_id = powerPortState.get("deviceId")
                if port_device_id and port_device_id != "controller":
                    if looks_like_feeder_device_id(port_device_id):
                        ensure_feeder_registered(port_device_id, reason="PowerPortState").last_seen = time.time()
        ...
        try:
            ...
            for sample in ac_MeasurementUpdateReader.samples.valid_data_iter:
                ...
                if not matched_source and devID != "controller" and looks_like_feeder_device_id(devID):
                    ensure_feeder_registered(devID, reason="ACMeasurementUpdate").last_seen = time.time()
            ...
            print(f"ACMeasurementUpdateReader error: {e}")
        ...
        try:
            ...
            for sample in replyReader.samples.valid_data_iter:
                ...
                reply_request = replyReaderUpdate.get("requestId", {})
                reply_target = reply_request.get("targetDeviceId") or replyReaderUpdate.get("deviceId")
                if reply_target and reply_target != "controller":
                    if looks_like_feeder_device_id(reply_target):
                        ensure_feeder_registered(reply_target, reason="Reply").last_seen = time.time()
