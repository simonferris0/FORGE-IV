...
OPEN_CONFIRM_CURRENT_MAX_A = 0.50
OPEN_CONFIRM_POWER_MAX_KW = 0.10
...
class TypeVControllerInterface:
    ...
    def _publish_controller_diagnostics(self) -> None:
        ...
        try:
            # Keep this minimal because the DDS schema in your environment does not
            # expose a freeform "message" field on ActiveDiagnosticState.
        ...
        for msg in self._safe_take(self.heartbeatReader):
            ...
            if self.feeder and device_id == self.feeder.device_id:
                ...
                if self.loop_counter <= 10:
                    print(f"[DISCOVERY] Feeder heartbeat candidate: {device_id} seq={feeder.heartbeat_seq}")
    ...
    def _read_ac_measurement_update(self) -> None:
        ...
        for msg in self._safe_take(self.acMeasurementUpdateReader):
            ...
            if self.feeder and device_id == self.feeder.device_id or self._looks_like_junction_node(device_id):
                ...
                if self.loop_counter <= 10:
                    print(f"[DISCOVERY] Feeder measurement candidate: {device_id} port={port_number}")
    ...
    def _diagnose_microgrid(self) -> None:
        ...
        if f.last_switch_command == "OPEN" and (time.time() - f.last_switch_time) > RESTORE_SETTLE_SEC:
            significant_current_present = any(abs(f.line_current.get(phase, 0.0)) >= OPEN_CONFIRM_CURRENT_MAX_A for phase in ["A", "B", "C"])
            significant_real_power_present = abs(f.total_real_kw) >= OPEN_CONFIRM_POWER_MAX_KW
            if significant_current_present and significant_real_power_present:
    ...
    def _print_status_summary(self) -> None:
        ...
        if self.feeder is None:
            ...
            print(
                "Feeder electrical: "
                f"Va={f.line_voltage.get('A', 0.0):.1f}V "
                f"Vb={f.line_voltage.get('B', 0.0):.1f}V "
                f"Vc={f.line_voltage.get('C', 0.0):.1f}V "
                f"Vavg={f.avg_voltage:.1f}V "
                f"Ia={f.line_current.get('A', 0.0):.2f}A "
                f"Ib={f.line_current.get('B', 0.0):.2f}A "
                f"Ic={f.line_current.get('C', 0.0):.2f}A "
                f"FreqA={f.line_frequency.get('A', 0.0):.2f}Hz "
                f"FreqB={f.line_frequency.get('B', 0.0):.2f}Hz "
                f"FreqC={f.line_frequency.get('C', 0.0):.2f}Hz "
                f"FreqAvg={f.avg_frequency:.2f}Hz "
