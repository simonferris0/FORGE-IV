...
class TypeVControllerInterface:
    def __init__(self, xml_path: Optional[str] = None):
        ...
        self.loads: Dict[str, DeviceState] = {}
        self.storage: Dict[str, DeviceState] = {}
        self.other_devices: Dict[str, DeviceState] = {}
    ...
    def _publish_controller_diagnostics(self) -> None:
        ...
        try:
            self.activeDiagnosticStateWriter.instance.set_string("deviceId", "controller")
    ...
    def _read_device_info(self) -> None:
        for msg in self._safe_take(self.deviceInfoReader, wait_ms=50 if self.loop_counter < 3 else 0):
            ...
            if device_id.startswith("hps-"):
                print(f"[DISCOVERY] Simulator device: {device_id} role={role}")
            ...
            if role in (ROLE_LOAD_NUM, ROLE_LOAD_STR):
                load = self.loads.get(device_id)
                if load is None:
                    load = DeviceState(device_id=device_id, role="LOAD")
                    self.loads[device_id] = load
                    print(f"[DISCOVERY] Load: {device_id}")
                continue

            if role in (ROLE_STORAGE_NUM, ROLE_STORAGE_STR):
                store = self.storage.get(device_id)
                if store is None:
                    store = DeviceState(device_id=device_id, role="STORAGE")
                    self.storage[device_id] = store
                    print(f"[DISCOVERY] Storage: {device_id}")
                continue

            if device_id not in self.other_devices:
                self.other_devices[device_id] = DeviceState(device_id=device_id, role=str(role))
                print(f"[DISCOVERY] Other device: {device_id} role={role}")
    ...
    def _read_ac_measurement_update(self) -> None:
        ...
        for msg in self._safe_take(self.acMeasurementUpdateReader):
            ...
            if device_id in self.loads:
                load = self.loads[device_id]
                load.last_measurement = now
                load.real_kw = self._sum_real_kw(lines)
                load.reactive_kvar = self._sum_reactive_kvar(lines)
                continue

            if device_id in self.storage:
                store = self.storage[device_id]
                store.last_measurement = now
                store.real_kw = self._sum_real_kw(lines)
                store.reactive_kvar = self._sum_reactive_kvar(lines)
                continue
    ...
    def _looks_like_junction_node(self, device_id: str) -> bool:
        ...
        return ("junction-node" in lowered or "junction_node" in lowered or lowered.startswith("jn-") or "sbc" in lowered) and not lowered.startswith("hps-")
    ...
    def _print_status_summary(self) -> None:
        ...
        if self.loads:
            ...
            for load in self.loads.values():
                print(
                    f"  - {load.device_id}: P={load.real_kw:.3f}kW Q={load.reactive_kvar:.3f}kVAR"
                )
        ...
        if self.storage:
            ...
            for store in self.storage.values():
                print(
                    f"  - {store.device_id}: P={store.real_kw:.3f}kW Q={store.reactive_kvar:.3f}kVAR"
                )

        if self.other_devices:
            print("Other devices:")
            for dev in self.other_devices.values():
                print(f"  - {dev.device_id}: role={dev.role}")
        print("==========================================================\n")
